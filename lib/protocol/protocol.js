...
function _encodeFetchRequest(clientId, correlationId, payloads, maxWaitMs, minBytes)
{
  ...
  topics.forEach(function (topic)
  {
    ...
    var partitions = _.pairs(payloads[topic]).map(function (pairs)
    {
      return pairs[1]
    });
  ...
  function decodePartitions(end, vars)
  {
    ...
    this.word32bs('partition')
      ...
      .tap(function (vars)
      {
        ...
        if (vars.errorCode !== 0)
        {
          cb({ topics : topics, topic : vars.topic, partition : vars.partition, message : ERROR_CODE[vars.errorCode] });
        }

        (function (_topic, _partition, _messageSet, _cb, _maxTickMessages)
        {
          callsPartitions.push(function (cbPartitionDone)
          {
            decodeMessageSet(_topic, _partition, _messageSet, _cb, _maxTickMessages, false, function (messageSet, topic, partition)
            {
              cbPartitionDone(null, [messageSet, topic, partition]);
            });
          });
        })(vars.topic, vars.partition, vars.messageSet, cb, maxTickMessages);

      });
  }

  /*** ASYNC DECODE MESSAGESET ***/
  async.parallel(callsPartitions, function (err, results)
  {
    results.forEach(function (aInfoMessageSet)
    {
      if (aInfoMessageSet[0].length)
      {
        var offset = aInfoMessageSet[0][aInfoMessageSet[0].length - 1]; // dernier offset
        topics[aInfoMessageSet[1]][aInfoMessageSet[2]] = offset;
      }
    });
...
function decodeMessageSet(topic, partition, messageSet, cb, maxTickMessages, bIncluded, cbMessageSetDone)
{
  var set = [];
  var calls = [];
  var messageCount = 0;
    ...
    Binary.parse(messageSet)
      ...
      .tap(function (vars)
      {
        if (vars.value !== -1)
        {
          ...
          //SNAPPY SNAPPY
          var codec = vars.attributes & 0x03;
          ...
          {
            if (detect_xerial_stream(vars.value))
            {
              var compressedBuffer = vars.value.slice(16);

              var len = compressedBuffer.length,
                cursor = 0;
              ...
              while (cursor < len)
              {
                try
                {
                  var blockSize = compressedBuffer.readUInt32BE(cursor);
                  cursor += 4;
                  var end = cursor + blockSize;
                  (function (val)
                  {
                    calls.push(function (callback)
                    {
                      snappy.uncompress(val, { asBuffer : true }, function (err, original)
                      {
                        if (err)
                        {
                          console.log('err', err);
                  ...
                  })(compressedBuffer.slice(cursor, len));
                }
                catch (e)
                {
                  //message not valid, possibly trying to access beyond buffer length
                  console.log(e);
                  // throw e;
                }
            ...
            else
            {
              calls.push(function (callback)
              {
                snappy.uncompress(vars.value, { asBuffer : true }, function (err, original)
                {
                  if (err)
                  {
                    console.log('err', err);
  ...
  if (calls.length)
  {
    ...
    {
      var includedMessageSet = [];

      results.forEach(function (value)
      {
        if (value)
        {
          decodeMessageSet(topic, partition, value, cb, maxTickMessages, true, function (messageSet)
          {
            messageSet.forEach(function (offset)
            {
              includedMessageSet.push(offset);
            });
          });
        }
      });
      cbMessageSetDone(includedMessageSet, topic, partition);
    });
  }
  else
  {
    // sync way
    cbMessageSetDone(set, topic, partition);
  }
}

function detect_xerial_stream(data)
{
  return data[0] == 130 //-126
    && data[1] == 83 //S
    && data[2] == 78 //N
    && data[3] == 65 //A
    && data[4] == 80 //P
...
function _encodeProduceRequest(clientId, correlationId, payloads, requireAcks, ackTimeoutMs)
{
  ...
  topics.forEach(function (topic)
  {
    ...
    var reqs = _.pairs(payloads[topic]).map(function (pairs)
    {
      return pairs[1]
    });
